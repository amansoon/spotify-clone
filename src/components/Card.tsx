  ...
  return (
    <div className="group min-h-[250px] transition-all bg-[#181818] hover:bg-[#242424]  p-4 rounded-[6px] cursor-pointer">
      ...
      <div className="min-h-[62px]">
        ...
        {type === CardType.Artist ? <ArtistDesc text={'Aman Ghanghoriya'} /> : null }
        {type === CardType.Playlist ? <PlaylistDesc text={'Aman Ghanghoriya'} /> : null }
        {type === CardType.Episode && (
          <CardDate date="Jan 23" duration="18 Min" />
        )}
      </div>
    </div>
  );
...
const CardTitle = ({ text, url }: TitleProps) => {
  return (
...
const CardDesc = ({ text, url }: DescProps) => {
  return (
    <div className="text-gray">
      <Link to="" className="text-sm leading-5">
        {text}
      </Link>
    </div>
  );
...
// Playlist desc
const ArtistDesc = ({ text }: {text: string}) => {
  return (
    <div className="text-gray">
...
// Playlist desc
const PlaylistDesc = ({ text }: {text: string}) => {
  return (
    <div className="text-gray">
      <span className="text-sm leading-5">
        By {text}
...
const CardDate = ({ date, duration }: CardDateProps) => {
  return (
